...
| `hub.refresh_discovery` | Refresh discovery | B | triggers worker; desktop only MVP |
| `hub.unlock_ports` | Unlock ports | B | smart release; desktop only MVP |
| `hub.qos_display` | QoS on card | RO | `GET /status` aggregates Hz/drops |
| `serial.com` | COM combo | MVP | `config.com_port` |
| `serial.baud` | Baud field | MVP | `config.baud` |
...
| `net.advanced_tcp` | Advanced TCP/UDP | B | |
...
| `field.refresh` | Refresh | B | |
| `field.unlock` | Unlock | B | |
...
| `web.settings` | Web enable/port | MVP | `ui_prefs` + future `GET /meta` |

## MVP coverage summary

- **In scope**: start, stop, status, core config (COM, baud, UDP listen, NMEA mode, hub id).
- **Explicitly deferred**: discovery refresh, unlock, fan-out, NTRIP, TCP advanced, presets write.
